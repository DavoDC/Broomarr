...
So Broomarr enumerates. It calls `/api/v3/episode?seriesId=N`, walks the list, and classifies each episode itself: which aired episodes have no file, which have not aired yet, and the identifiers of every episode that does have one. An episode counts as missing when it has aired and has no file, **regardless of whether it is monitored** - an unmonitored episode is one Sonarr will never fetch, which makes it more permanently unwatchable, not less.
...
- **A cheap prefilter runs first.** Enumerating every series in a large library is a request per show, so `scan()` runs a cheap pass over the whole library before that request goes out. The cheap pass is the same verdict with the episode-list conditions skipped - it does not call Sonarr or look at any count, it just does not ask the question enumeration exists to answer. That is what makes it safely permissive: skipping a condition can only pass more shows than checking it would, never fewer, so the cheap pass can never block a show the strict pass would have let through. Only its survivors get the strict per-episode check. Fast and correct, in that order of discovery but not of priority.

## The third trap: a count is a set with the identities thrown away

The second trap says a count can carry a filter you did not choose. The third trap is more basic than that and does not need a hidden filter to bite: a count is a set with the identities thrown away, so comparing two counts is not a cheaper way of comparing two sets, it is a different question, and answering it says nothing about whether the two sets agree.

"The watcher watched 10 episodes and 10 episodes are on disk" does not establish that they are the same 10 episodes. A show can have a season sitting on disk that nobody watched while the totals agree exactly, because the watcher moved on to a later season instead of finishing the one they started. That is the shape of the case that matters: somebody midway through a show, the two figures equal, and a count-based check recommending deletion.

Broomarr shipped with this exact fault. `verdict()` decided whether the watcher had seen everything on disk by comparing `len(watched_eps)` to `stats["episodeFileCount"]` - two integers - in the same document that argues against counts two sections above. `episodeFileCount` is also the filtered count the second trap already discredits, so the comparison was wrong twice: once for being a count at all, and once for being that particular count. The argument in this file had been made about the missing-episodes check; the count was sitting in a different check, the watched-vs-on-disk one, where nobody was watching for it. The fix was to remove the count rather than correct it, because there was no version of "compare these two numbers" that answers the question being asked.

`episode_facts()` now returns the `(season, episode)` identifiers of every episode Sonarr has a file for, and `verdict()` takes the set difference between that and the identifiers Tautulli recorded as watched. Anything left over is an episode on disk nobody watched, named specifically rather than counted. Both sides of that difference are coerced to `int` first: Tautulli returns season and episode numbers as strings in some API versions, and comparing `"2"` to `2` unconverted would make the two sets never intersect at all, so every on-disk episode would read as unwatched. That fails closed rather than open - it blocks everything rather than passing the wrong thing - but it is also silent, and it would look exactly like a healthy check that never finds a finished show. A history row whose season or episode will not coerce to an int is dropped rather than counted as a watch, on the same unknown-blocks logic as everywhere else in this file.
